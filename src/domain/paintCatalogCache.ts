...
/**
 * Persistence for a catalogue fetched at runtime, so an update survives the
...
export type CachedCatalog = {
  /** Hash of the upstream HTML this catalogue was parsed from. */
  htmlHash: string;
  paints: Paint[];
};
...
type StoredCatalog = CachedCatalog & {
   ...
   * snapshot and we drop the cache — otherwise shipping a corrected catalogue
   * in a release would never reach anyone who had already cached the bad one.
   */
  bundledSignature: string;
};

/** Identity of a catalogue's contents, for equality checks only. */
...
/**
 * The cached catalogue, or undefined if there is none we can trust. Signing
...
export function writeCachedCatalog(bundledPaints: Paint[], entry: CachedCatalog): void {
  ...
  try {
    ...
    // Out of quota or storage unavailable. The refreshed catalogue still
