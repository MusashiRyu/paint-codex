# Retro 014 — The catalog's real source, and 310 paints became 2,279
...
940 Vallejo, 536 Army Painter. Color, range, opacity and ΔE live on the
...
Old catalog: 310 paints, of which 206 had no `category` at all. New: 2,279, covering
...
replaces the whole catalog, so any paint the merge added would have vanished
...
Upstream ships color, not conversions. `attachMatches` converts every paint to
...
matches for the paints in its table, so most of the catalog had none.
...
at different colors. The old `Match` carried `{brand, name, hex, delta}` and
...
and resolves in one pass, and drops an id whose paint has left the catalog —
...
of the same brand and name whose color is nearest the one the old snapshot
carried — nearest, because the old catalog stored one hex per name and it is
...
break preferring brush ranges over airbrush ones: the old catalog was built
from redgrimm's Citadel "Base Layer" and Vallejo Game/Model ranges — brush paint
throughout — so when a name exists in both Air and Base at the same color,
...
and they were deliberately left unmapped: a color-nearest fallback would have
...
comfortably over a floor of 1,139. The refresh would have adopted a catalog
...
cases now cover unique ids, renderable colors, every match resolving inside the
same file, no same-brand equivalents, no two entries sharing a brand, name and
color, no paint being shown two tiles that read identically, and every
...
wrong one for paints that do not. Upstream lists a color once per range it is
sold in, so Citadel's Abaddon Black arrived as both Air and Base at `#000000`
and Vallejo's Black three times over. 143 rows were a brand, name and color
...
- **492 paints — 20% of the catalog — had equivalent tiles that repeated.**
...
The fix is one entry per brand + name + color, whose `category` lists every
...
color stays a separate paint, which is what the original decision was actually
about: Administratum Grey really is a different gray in Layer and in Air.
...
someone browsing a color usually wants, and it is what the old catalog held.
...
fails, alongside a check that every target exists in the catalog.
...
Administratum Grey differ in color. Only the second is a variant to a person
holding a pot of paint, and the rule that separates them is just "is any
rendered field different".

## Not done

- **Screenshots were not regenerated.** The catalog content and the equivalent
